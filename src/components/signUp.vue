        ...
        <form class="login-form" id="loginForm" novalidate>
          <div class="form-group">
            ...
            <div class="input-wrapper">
              <input v-model="formSign.name" type="text" id="name" name="name" required />
          ...
          <button @click="signIn" type="button" class="login-btn btn">
...
const signIn = async () => {
  try {
    const res = await axios.post("http://localhost:3000/user", formSign.value);
    signInData.value = res.data;
    router.push("/home");
    localStorage.setItem("userData", JSON.stringify(signInData.value));
  } catch (error) {
    console.error("Error during sign-in:", error);
    alert("Error during sign-in: " + (error?.message || error));
  }
};
...
.main-container {
  ...
  background: linear-gradient(135deg, #95aff7 0%, #91b1f3 100%);
  min-height: 100vh;
  display: flex;
